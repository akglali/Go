...
func PostSinglePostDb(token, nickname, textField, currentTime, color string) (error, *sql.Row) {
	...
	_, err = database.Db.Exec("insert into post_user_nickname_table (post_id, user_id, nickname, color) values ((select user_id from users where token=$2),$1,$3,$4)", postId, token, nickname, color)
	if err != nil {
		return err, nil
	}
	return err, row
